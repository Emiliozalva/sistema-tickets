...
export default function PanelArea() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (imagen) {
        setMensaje("Subiendo captura de pantalla...");
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <main className="bg-white p-7 rounded-2xl shadow-sm border border-gray-100">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col">
            <label className="text-sm font-medium text-gray-600 mb-1">Adjuntar captura (Opcional):</label>
            <input type="file" accept="image/*" onChange={(e) => setImagen(e.target.files[0])} className="p-2 border border-gray-300 rounded-lg text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-amarillo-pastel file:text-yellow-800 hover:file:bg-amarillo-vivo transition-all" />
          </div>
